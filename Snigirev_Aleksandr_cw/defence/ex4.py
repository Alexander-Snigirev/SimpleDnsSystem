...
def backtrack(grid: List[List[int]], squares: List[Tuple[int, int, int]], N: int, best_solution: List[Tuple[int, int, int]]):
    """Рекурсивный поиск с возвратом для нахождения минимального разбиения квадрата."""
    
    # Если уже найдено лучшее решение с меньшим количеством квадратов — прерываем поиск
    ...
    # Найти первую свободную клетку (нулевую) в сетке
    ...
    # Если вся сетка заполнена — сохраняем лучшее решение
    if min_x is None:
    ...
    # Пробуем размещать квадраты от максимального возможного размера до 1 (но не N)
    max_size = min(N - min_x, N - min_y, N//2+1)  # Ограничение на макс. размер квадрата
    for size in range(max_size, 0, -1):
        if is_valid(grid, min_x, min_y, size, N):
            ...
            # Откатываемся, если текущий путь не дал оптимального решения
            squares.pop()
...
def squaring_the_square(N: int) -> List[Tuple[int, int, int]]:
    ...
    if N%2==0:
        return [(0, 0, N/2), (0, N/2, N/2), (N/2, 0, N/2), (N/2, N/2, N/2)]
    if N%3==0:
        return [(0, 0, N*2/3), (0, N*2/3, N/3), (N/3, N*2/3, N/3), (N*2/3, 0, N/3), (N*2/3, N/3, N/3), (N*2/3, N*2/3, N/3)]
...
def visualize_solution(N: int, solution: List[Tuple[int, int, int]]):
    ...
    for x, y, size in solution:
        ...
        ax.add_patch(rect)
        ax.text(y + size / 2, N - x - size / 2, str(size), 
                ha='center', va='center', fontsize=12, color='white', weight='bold')

    plt.title(f"Разбиение квадрата {N}x{N} на минимальные квадраты")
    plt.savefig('go.png')

# Проверка работы на квадрате 6x6

N = int(input())
solution = squaring_the_square(N)
print("Минимальное разбиение квадрата ", solution)
